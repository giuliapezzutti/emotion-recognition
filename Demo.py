...
index = np.random.randint(0, ref_wind.shape[0])
data_dir = 'data'
...
fig.suptitle('{} - Before and after normalization'.format(index))
axs[0].plot(data_nonorm[0, :], color='b')
axs[1].plot(data[0, :, 0], color='r')
plt.savefig('images/datanormalized.png')
...
model = tf.keras.models.load_model('models/arousal/SC_DNNCNN.h5')

test = ref_wind.loc[[index], :]
test_label = arousal_label

test_dataset = create_dataset('Arousal_label', test, function=load_SC,
...
print('Estimated arousal label: {}'.format(test_est_class))


model = tf.keras.models.load_model('models/both/SC_DNNCNN.h5')

test = ref_wind.loc[[index], :]
test_label = np.array((arousal_label, valence_label))
